...
def login():
    if request.method == 'POST':
        if request.form.get("login"):
            ...
            try:
                cursor.execute("SELECT full_name FROM service.users WHERE login=%s AND password=%s",
...
def registration():
    if request.method == 'POST':
        ...
        l0gin = request.form.get('login')
        password = request.form.get('password')
        if (not name) or (not l0gin) or (not password):
            return render_template('emptyfield.html')
        ...
        cursor.execute('INSERT INTO service.users (full_name, login, password) VALUES (%s, %s, %s);',
                       (str(name), str(l0gin), str(password)))
        conn.commit()
        return redirect("/login/")
